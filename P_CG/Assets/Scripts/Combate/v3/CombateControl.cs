...
public class CombateControl : MonoBehaviour
{
    ...
    protected virtual IEnumerator AtaqueEnemigo(int idx)
    {
        ...
        if (combinado > 50 && combinado < 90)
        {
            ...
            int ataqueAleatorio = Random.Range(0, enemigo.data.ataques.Length);
            int playerObjetivo = Mathf.Clamp(PlayerSelect, 0, cantidadPlayers - 1);
            yield return StartCoroutine(AplicarDanoEnemigo(enemigo, ataqueAleatorio, playerObjetivo));
        }
        else
        {
    ...
    protected IEnumerator AplicarDanoEnemigo(Character enemigo, int indiceAtaque, int indicePlayer)
    {
        if (enemigo == null || indiceAtaque >= enemigo.data.ataques.Length) yield break;
        if (indicePlayer >= cantidadPlayers) yield break;

        // Verificar que el GameObject del player sigue existiendo
        Transform playerTransform = players.transform.GetChild(indicePlayer);
        if (playerTransform == null) yield break;
        Character player = playerTransform.GetComponent<Character>();
